...
void print_hex(const unsigned char *bytes, unsigned long long len)
{
...
{
	// Define inputs
	unsigned char npub[12] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B};								   // Nonce
	unsigned char key[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};		   // Secret key
	unsigned char ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11}; // Additional data
	unsigned char message[] = {"Temperature=52"};																					   // Message

	unsigned long long adlen = strlen((const char *)ad);
	unsigned long long mlen = strlen((const char *)message);
	unsigned long long clen;

	// Encrypt the message
	unsigned char ciphertext[mlen + 16]; // Allocate space for ciphertext
	int encrypt_result = crypto_aead_encrypt(ciphertext, &clen, message, mlen, ad, adlen, NULL, npub, key);
	if (encrypt_result != 0)
	{
	...
	int decrypt_result = crypto_aead_decrypt(plaintext, &decrypted_mlen, NULL, ciphertext, clen, ad, adlen, npub, key);
	if (decrypt_result != 0)
	{
